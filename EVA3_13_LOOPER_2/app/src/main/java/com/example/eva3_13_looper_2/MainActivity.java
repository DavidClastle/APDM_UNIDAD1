    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        background.ejecutarTarea(new Runnable() {
            ...
            public void run() {
                ...
                Message message = handler.obtainMessage(100, "FIN HILO A");
                handler.sendMessage(message);
            }
...
class Background extends HandlerThread{
    Handler handler;
    public Background() { //por herencia vamos a dar constructor al handler thread
        super("Background");
        start();
        handler = new Handler(getLooper());
    }
    public Background ejecutarTarea(Runnable tarea){
        handler.post(tarea); //enviar tarea que quiero ejecutar y mostrar en interfaz grafica
        return this; //regresar el mismo obj
    }
}